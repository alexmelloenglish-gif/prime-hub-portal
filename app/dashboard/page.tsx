...
function reportLearningNotes(value: string) {
  ...
  const labels = ['Evidence', 'Interpretation', 'Boundary', 'Next verification'] as const
  ...
  for (const label of labels) {
    ...
    const nextPositions = labels
      ...
      .map((candidate) => body.indexOf(`${candidate}:`, contentStart))
      .filter((position) => position >= 0)
    const end = nextPositions.length ? Math.min(...nextPositions) : body.length
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        {showWhatChanged ? (
          <article className="rounded-[24px] border border-blue-300 bg-blue-50 p-5 shadow-sm ring-1 ring-blue-100">
            <div className="flex items-start justify-between gap-3"><Sparkles className="h-5 w-5 text-blue-700" /></div>
        ...
        <article className="rounded-[24px] border border-amber-200 bg-amber-50/70 p-5 shadow-sm ring-1 ring-amber-100">
          ...
          <div className="mt-5 space-y-3">
            ...
            }) : <p className="text-sm text-slate-600">Your next learning focus will appear here when it is ready.</p>}
          </div>
        </article>
      </section>
