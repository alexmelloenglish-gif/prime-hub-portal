  ...
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white p-5 text-[#0a235c] shadow-[0_16px_42px_rgba(15,48,93,0.07)] md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#7184a1]">Lesson history</p>
      ...
      {requestedStudent ? (
        <div className="mt-4 flex flex-wrap gap-2">
          <Link href="/dashboard/admin/intelligence/students" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-[#263c86] hover:bg-indigo-50">
            Back to learners
          </Link>
          <Link href={`/dashboard?studentEmail=${encodeURIComponent(requestedStudent)}`} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-[#263c86] hover:bg-indigo-50">
            Student dashboard
          </Link>
        </div>
      ) : null}
      ...
      {lessons.length ? (
        <div className="mt-5 space-y-4">
          {lessons.map((attempts) => {
            ...
            const evidenceCount = Math.max(...attempts.map((attempt) => attempt.evidenceCount), 0)
              ...
              <article key={`${latest.studentEmail}:${latest.lessonId}`} className="rounded-2xl border border-slate-200 bg-[#fbfdff] p-4">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 break-all text-xs text-[#60718d]">Lesson {latest.lessonId}</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {attempts.length > 1 ? (
                      <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-[#60718d]">
                        {attempts.length} processing attempts
                      </span>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-200 pt-4">
                  <Link
                    href={`/dashboard/admin/intelligence/lessons/${latest.pipelineRunId}`}
                    className="inline-flex items-center gap-1 rounded-lg border border-indigo-100 bg-indigo-50 px-3 py-2 text-xs font-bold text-indigo-700 hover:bg-indigo-100"
                  >
                    Open lesson details <ArrowRight className="h-3.5 w-3.5" aria-hidden="true" />
                  </Link>
                </div>

                <details className="mt-4 rounded-xl border border-slate-200 bg-white">
                  <summary className="cursor-pointer px-4 py-3 text-xs font-bold text-[#60718d]">
                    Processing history · {attempts.length} attempt{attempts.length === 1 ? '' : 's'}
                  </summary>
                  <div className="space-y-2 border-t border-slate-200 p-3">
                    {attempts.map((attempt, index) => (
                      <div key={attempt.pipelineRunId} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-[#fbfdff] p-3 lg:flex-row lg:items-center lg:justify-between">
                        <div className="min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <IntelligenceStatusBadge
                            label={attempt.technicalStatus === 'failed' ? 'processing failed' : `processing ${attempt.technicalStatus}`}
                          ...
                          <Link
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs font-bold text-[#60718d] hover:bg-slate-50"
                          >
                            Technical trace
                          </Link>
                        </div>
        ...
        <div className="mt-5 flex items-center gap-3 rounded-2xl border border-slate-200 bg-[#f8fbff] p-4 text-sm text-[#60718d]">
          ...
          {requestedStudent ? 'No lesson history is available for this learner.' : 'No lesson history is available yet.'}
        </div>
      )}
    </section>
  )
