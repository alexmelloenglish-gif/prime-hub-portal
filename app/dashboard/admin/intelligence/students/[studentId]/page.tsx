    ...
    <section className="space-y-5">
      ...
      <header className="rounded-[28px] border border-emerald-200 bg-white p-5 shadow-[0_16px_42px_rgba(15,48,93,0.07)] md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-700">Teacher-authorized V2 package</p>
          ...
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
            <div className="flex items-center gap-2 font-bold"><ShieldCheck className="h-4 w-4" aria-hidden="true" /> Teacher authority recorded</div>
            <p className="mt-1 text-xs">{pkg.teacher.name} · {pkg.decisionDate}</p>
          </div>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-[#f8fbff] p-4"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#7184a1]">State / priority</p><p className="mt-2 font-bold text-[#0a235c]">V2 update accepted</p></div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-[#f8fbff] p-4"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#7184a1]">Projection</p><p className="mt-2 font-bold text-[#0a235c]">Authorized for canonicalization</p></div>
        </div>
      </header>

      <section className="rounded-[26px] border border-indigo-100 bg-white p-5 shadow-sm md:p-6">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo-600">Exception-based authority</p>
        <h2 className="mt-1 text-2xl font-bold text-[#0a235c]">The system does the evidence work. The teacher decides material transitions.</h2>
        <p className="mt-3 max-w-4xl text-sm leading-7 text-[#526783]">{pkg.governance.rule}</p>
        <p className="mt-2 text-sm leading-7 text-[#526783]">Source-grounded extraction, provenance and candidate pattern detection do not require individual teacher clicks. Human authority is applied at the bounded state / priority / next-action transition.</p>
      </section>

      <section className="space-y-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#7184a1]">Source-grounded rebuild</p>
          <h2 className="mt-1 text-2xl font-bold text-[#0a235c]">Four lessons reprocessed from original source</h2>
        </div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {pkg.sourceLessons.map((lesson) => (
            <article key={lesson.sourceDocumentId} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-2 break-all text-xs leading-5 text-[#60718d]">Source {lesson.sourceDocumentId}</p>
              <span className="mt-3 inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.12em] text-emerald-800">source grounded</span>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2"><FileText className="h-5 w-5 text-indigo-600" aria-hidden="true" /><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo-600">Class Reports V2</p><h2 className="text-2xl font-bold text-[#0a235c]">Evidence → Signal → Interpretation → Boundary → Verification</h2></div></div>
        <div className="space-y-4">
          {pkg.classReportsV2.map((report) => (
            <article key={report.date} className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-indigo-600">{report.date}</p><h3 className="mt-1 text-xl font-bold text-[#0a235c]">{report.title}</h3></div><span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-800">Teacher-authorized package</span></div>
      ...
      <section className="rounded-[26px] border border-indigo-100 bg-white p-5 shadow-sm md:p-6">
        <div className="flex items-center gap-2"><Waypoints className="h-5 w-5 text-indigo-600" aria-hidden="true" /><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo-600">Accepted longitudinal signals</p><h2 className="text-2xl font-bold text-[#0a235c]">Patterns authorized for teacher use</h2></div></div>
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          {pkg.acceptedLongitudinalSignals.map((item) => (
            <article key={item.id} className="rounded-2xl border border-slate-200 bg-[#fbfdff] p-4">
              <div className="flex gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-600" aria-hidden="true" /><div><p className="font-bold text-[#0a235c]">{item.signal}</p><p className="mt-2 text-sm leading-6 text-[#60718d]">Boundary: {item.boundary}</p></div></div>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <article className="rounded-[26px] border border-blue-100 bg-white p-5 shadow-sm md:p-6">
          <div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-blue-600" aria-hidden="true" /><h2 className="text-xl font-bold text-[#0a235c]">Authorized current package</h2></div>
        ...
        <article className="rounded-[26px] border border-emerald-100 bg-white p-5 shadow-sm md:p-6">
          <div className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-emerald-600" aria-hidden="true" /><h2 className="text-xl font-bold text-[#0a235c]">Authorized next action</h2></div>
      ...
      <section className="rounded-[26px] border border-amber-200 bg-amber-50 p-5 md:p-6">
        <div className="flex gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" aria-hidden="true" /><div><p className="font-bold text-amber-950">Claims that remain outside automatic promotion</p><div className="mt-3 space-y-2">{pkg.nonAutoPromotableClaims.map((item) => <p key={item} className="text-sm leading-6 text-amber-900">• {item}</p>)}</div></div></div>
      </section>
    </section>
  )
}
