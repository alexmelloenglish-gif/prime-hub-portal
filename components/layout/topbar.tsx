  ...
  return (
    <div className="sticky top-0 z-20 flex items-center justify-between border-b border-white/10 bg-prime-dark/80 px-4 py-4 backdrop-blur md:px-6">
      ...
      <div className="flex items-center gap-4">
        <button type="button" className="rounded-lg p-2 transition-colors hover:bg-white/10" aria-label="Notificações">
          <Bell className="h-5 w-5 text-prime-cream/70" />
        </button>

        <div className="flex items-center gap-3 border-l border-white/10 pl-4">
          <div className="text-right">
            <p className="text-sm font-medium text-white">{user.name || 'Aluno'}</p>
            <p className="text-xs text-prime-cream/60">{user.email || 'Sem e-mail'}</p>
          </div>
          {user.image ? (
            <Image
              src={user.image}
