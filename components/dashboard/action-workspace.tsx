...
export function ActionWorkspace({ title, description, vocabulary, studentEmail, actionId, materialUrl, bookingUrl }: Props) {
  ...
  if(submitted)return <div className="space-y-5">
    ...
    <article className="rounded-[28px] border border-sky-300/20 bg-sky-300/5 p-6">
      ...
      {bookingUrl?<a href={bookingUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-semibold text-[#0a235c]">Book your next class <ArrowRight className="h-4 w-4"/></a>:null}
    </article>
  </div>
