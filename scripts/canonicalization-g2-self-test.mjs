...
const runtimeProof = fs.readFileSync('lib/g2-runtime-proof.ts', 'utf8')
...
assert(runtimeProof.includes("canonicalizeLearningRecord(command)"), 'runtime proof must execute canonicalization through the G2 service')
assert(runtimeProof.match(/canonicalizeLearningRecord\(command\)/g)?.length === 2, 'runtime proof must replay the same canonicalization command exactly twice')
assert(runtimeProof.includes("canonicalRecords !== 1"), 'runtime proof must enforce one canonical record')
assert(runtimeProof.includes("provenanceRows !== 1"), 'runtime proof must enforce one provenance row')
assert(runtimeProof.includes("G2_CONTROLLED_ROLLBACK"), 'runtime proof must contain a controlled rollback failure point')
assert(runtimeProof.includes("noPartialState"), 'runtime proof must verify no partial canonical state survives rollback')
assert(!runtimeProof.includes("buildLearningIntelligence("), 'runtime proof must not invoke Learning Intelligence')
assert(!runtimeProof.includes("buildDashboardProjection("), 'runtime proof must not invoke Dashboard projection')
assert(!runtimeProof.includes("ClassReportProjection"), 'runtime proof must not invoke Class Report projection')
