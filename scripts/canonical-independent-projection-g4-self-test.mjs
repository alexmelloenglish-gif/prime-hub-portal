...
assert(service.includes('originalSourceReferences'), 'G4 projection must preserve original canonical sourceReferences')
assert(service.includes('teacherDecisionId'), 'G4 projection must preserve teacherDecisionId')
assert(service.includes('validationTaskId'), 'G4 projection must preserve ValidationTask lineage when resolvable')
assert(service.includes('teacherDecisionPackageId'), 'G4 projection must preserve Teacher Decision Package lineage when resolvable')
assert(service.includes('authorityScope'), 'G4 projection must preserve authorityScope')
assert(service.includes('unresolvedBoundary'), 'G4 projection must preserve unresolved legacy boundary markers')
assert(service.includes('sourceReferences,\n    projection,'), 'G4 projection hash must bind canonical lineage and projected payload')
...
assert(runtimeProof.includes('const first = await projectCanonicalPortfolio(input)'), 'G4 runtime proof must execute the first projection')
assert(runtimeProof.includes('const replay = await projectCanonicalPortfolio(input)'), 'G4 runtime proof must execute an exact replay')
assert(runtimeProof.includes('canonicalLearningRecordProjection.count'), 'G4 runtime proof must count persisted projections')
assert(runtimeProof.includes("if (!replay.idempotentReplay)"), 'G4 runtime proof must assert idempotent replay')
assert(runtimeProof.includes('if (projectionCount !== 1)'), 'G4 runtime proof must assert exactly one persisted projection')
