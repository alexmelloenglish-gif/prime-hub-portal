...
assert.match(eligibility, /emailAliases/)
const gustavo = registry.students.find((student) => student.studentId === 'stu_4c4da6c04ac4')
assert.ok(gustavo?.emailAliases?.includes('gugasalgado7@gmail.com'), 'Gustavo learner login must be an explicit authorized compatibility alias')
const studentData = fs.readFileSync(path.join(root, 'lib/student-data.ts'), 'utf8')
assert.match(studentData, /'gugasalgado7@gmail.com': gustavoProfile/)
