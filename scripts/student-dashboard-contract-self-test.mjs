...
assert.equal(eduarda.canonicalProjection.whatChanged.changeType, 'learning-priority')
...
assert.ok(dashboardSource.includes('What you worked on'))
assert.ok(dashboardSource.includes('What we noticed'))
assert.ok(dashboardSource.includes('What comes next'))
assert.ok(!dashboardSource.includes('Recent Report Highlights'))
assert.ok(dashboardSource.includes("whatChanged.changeType === 'learning-priority'"))
...
assert.ok(primitiveSource.includes('Your progress is reviewed through what you can do in lessons.'))
assert.ok(!primitiveSource.includes('Why this matters'))
...
const claudioMission = JSON.parse(await readFile(new URL('data/students/claudio-bit-gmail-com.firestore.json', root), 'utf8'))
