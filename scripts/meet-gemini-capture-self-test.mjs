...
assert(capture.includes('proven: false'), 'scheduled operator lesson identity remains explicitly unproven before Calendar correlation')
assert(capture.includes("source: 'authenticated_teacher_attestation'"), 'unscheduled lessons can use explicit authenticated teacher attestation')
assert(capture.includes("origin: 'unscheduled'"), 'unscheduled lesson origin is preserved in provenance')
assert(capture.includes("deliveryMode: input.deliveryMode"), 'capture persists lesson delivery mode')
assert(capture.includes("speakerDiarizationStatus: input.speakerDiarizationStatus"), 'capture persists diarization reliability')
assert(capture.includes('evidenceAttributionRequiresConfidence: true'), 'capture requires confidence-aware evidence attribution')
...
assert(generator.includes('Speaker attribution is evidence, never an assumption'), 'generator treats speaker attribution as evidence rather than identity')
assert(generator.includes('Never infer that a voice is human, AI, teacher, or student'), 'generator cannot claim human-vs-AI identity from conversational style')
...
assert(route.includes("body.deliveryMode === 'in_person'"), 'endpoint accepts in-person capture mode')
assert(route.includes("body.lessonOrigin === 'unscheduled'"), 'endpoint accepts unscheduled lesson origin')
assert(route.includes('teacherAttestedStudent'), 'endpoint accepts explicit teacher student attestation')
