...
const CANDIDATE_SHA =
  process.env.RUNTIME_WITNESS_EXPECTED_SHA
  || 'd29f87480d43168311deab635c93acfcb0137118'
const PR_NUMBER = Number(process.env.RUNTIME_WITNESS_PR_NUMBER || '64')
const POINTER =
  process.env.RUNTIME_WITNESS_POINTER
...
async function main() {
    ...
    (request, response) => {
      void (async () => {
        try {
          ...
          // candidate SHA and GitHub evidence location. No repository,
  ...
  try {
    ...
    const successInsert = await ingestAgentCoordinationEvent(eventInput({
      ...
      payload: {
        instruction: 'Validate the exact integration candidate pointer and ACK the same event.',
