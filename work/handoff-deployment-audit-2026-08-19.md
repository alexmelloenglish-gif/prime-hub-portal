...
## Loop incident: verified scanner and backend causality

The canonical `scripts/scan-drive-transcripts.mjs` is an operator-invoked scanner, not a persistent Google Workspace Studio trigger. It lists files in the configured folder, downloads Google Docs/plain text, triages them, and only submits when run with `--submit`. Its local state file skips a file only when the same file ID has a matching fingerprint and a prior `submitted` status; that state is not a durable production control and can be absent or reset between runs.

The scanner sends `metadata.sourceFileId = file.id`, `metadata.sourceDocumentId = file.id`, and `transcriptId = file.id`. The canonical backend now derives `drive:<sourceFileId>` as the idempotency key and checks for an existing transcript bound to that Drive ID before creating a run. However, the database schema still has no dedicated unique source-file column, and the backend guard depends on the incoming payload carrying the immutable source file ID. The Workspace Studio Flow remains unverified; the prior authenticated inspection found active flows for `Tactiq Transcription` and `SANTA CRUZ — ARQUIVO MESTRE`, not the PRIME Meet Recordings folder. Therefore the repository cannot explain the exact 200-file execution history without the Studio activity log, but it proves the intended automatic trigger was not implemented in the repository and that loop protection must exist both in the Flow and in the database-backed ingestion path.

The validator utility is a separate path and does not send `metadata.sourceFileId`; it uses ingestionId/contentHash. It must not be treated as equivalent Drive-trigger protection.

Source files: `scripts/scan-drive-transcripts.mjs`, `lib/pipeline/run.ts`, `prisma/schema.prisma`, `app/api/pipeline/ingest/route.ts`, and `work/google-studio-flow-inspection-2026-08-17.md`.
